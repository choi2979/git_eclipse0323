...
public class ServerBankThread extends Thread {
	...
	ObjectOutputStream 	oos 		= null;//말하다가 멈출 있으니까 try..catch
	String 				nickName 	= null;
	//생성자의 파라미터 자리에 있는 클래스와 의존관계에 있다.
	public ServerBankThread(ServerBank sb) {
		this.sb = sb;
		try {
			oos = new ObjectOutputStream(sb.socket.getOutputStream());//oos부터
	...
	public void login(String mem_id) {
		...
		if(rMap != null) {//로그인 성공했을 때
			...
			this.broadCasting(msg2);//100#진완
		}
		else {//실패 했을 때
			for(ServerBankThread sbt: sb.globalList) {
				
				this.send(100+"#"+mem_id+"님은 로그인 실패 입니다.");
			}
			this.broadCasting(100+"#"+mem_id+"님은 로그인 실패 입니다.");//100#진완
		}
	}
	public void broadCasting(String msg) {
		//현재 서버에 몇사람이 있는지 출력하기
		sb.jta_log.append("현재 인원수:"+sb.globalList.size()+"\n");
		synchronized (this) {//다른 스레드가 인터셉트 하는것을 방어하기 위해 동기화처리함
			for(ServerBankThread sbt:sb.globalList) {
				sbt.send(msg);//tst가 접속자를 구분
			}
